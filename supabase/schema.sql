...
-- A token belongs to whichever account is signed in on that device right
-- now - if someone signs out and a different person signs in on the same
-- phone, the upsert in registerPushToken() reassigns user_id rather than
-- leaving a stale second row for the same physical token.
...
-- Marketing: brand asset previews, task list, shared notes -----------------
